...
<v-layout row wrap>
  <v-row>
    ...
    <v-col cols="2">
        <v-tabs v-model="tab">
             <v-tab class="text-subtitle" v-for="(v, pp, idx) in getFilters" :key="idx">{{pp}} Filter</v-tab>
    </v-tabs>
    <v-window v-model="tab">
      <v-window-item v-for="(v, pp, idx) in getFilters" :key="idx">
        <v-card>
          <v-list
            subheader
